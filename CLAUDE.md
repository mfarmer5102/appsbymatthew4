...
The schema is snapshotted at `backend/ddl.sql` so drift shows up in diffs. It is a
reference export, not a migration — see the header in that file for the two ways it
differs from the live database. Re-export it after any schema change.

A star schema in the `apps_by_matthew` Postgres schema. Every dimension is addressed by an
integer surrogate key (`application_key`, `skill_key`, …) — there are no `code` columns;
that concept came from the MongoDB era and was dropped in the Supabase migration. The
bridges and the embedding table are keyed by composite natural keys instead.

All four dimension keys are `int4 GENERATED BY DEFAULT AS IDENTITY`, so INSERTs omit the
key and let Postgres assign it. `BY DEFAULT` (rather than `ALWAYS`) means an explicit key
can still be supplied, which is how bulk loads leave the sequence behind its own table —
`npm run check-db` checks for exactly that and prints the `setval` needed to fix it.
...
Every dimension also carries `created_at` / `updated_at`. Only `dim_application` and
`dim_support_status` default `created_at` to `now()`; on `dim_skill` and `dim_skill_type`
it is a plain nullable column. That inconsistency is inert today because the data layer
writes both timestamps explicitly on every INSERT and `updated_at = now()` on every
